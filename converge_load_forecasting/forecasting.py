...
class Customers:
    ...
    def __init__(self, nmi, data, input_features):
        ...
        Customers.instances.append(self)
...
def forecast_pointbased_multiple_nodes_2(customers: Dict[Union[int,str],Customers], input_features: Dict) -> pd.DataFrame:
    """
...
# def pool_executor_parallel_1(function_name, repeat_iter, new_index, exog_f, exog, f_steps, input_features):
#     '''
#     pool_executor_parallel(function_name,repeat_iter,input_features)
...
#     '''
#     with ProcessPoolExecutor(max_workers=input_features['core_usage'],mp_context=mp.get_context('fork')) as executor:
#         results = list(executor.map(function_name,repeat_iter,itertools.repeat(new_index),itertools.repeat(exog_f),itertools.repeat(exog),
#                                     itertools.repeat(f_steps),itertools.repeat(input_features)))  
#     return results
...
# # This function outputs the forecasting for each nmi
# def run_forecast_pointbased_single_node_1(customer_nmuber,input_features: Dict) -> pd.DataFrame:
#     """
#     forecast_pointbased_autoregressive_single_node(customers_nmi,input_features)
...
#     data = data_gg.loc[customer_nmuber]

#     exog, exog_f, f_steps, new_index = add_exog_for_forecasting_1(data, input_features)

#     s = time.time()
#     # Train a forecasting object
#     forecaster = generator_forecaster_object(data, input_features, exog)
#     e = time.time()
#     print(e-s)

#     s = time.time()
#     # Generate predictions 
#     predictions = generate_prediction(data, input_features, new_index, forecaster, exog_f, f_steps)
#     e = time.time()
#     print(e-s)

#     s = time.time()
#     result = generate_output_adjusted_format_for_predictions_1(predictions, customer_nmuber, input_features)
#     e = time.time()
#     print(e-s)
...
# def forecast_pointbased_multiple_nodes_1(data_g: Dict[Union[int,str],Customers], input_features: Dict) -> pd.DataFrame:
#     """
#     forecast_pointbased_autoregressive_multiple_nodes(customers_nmi,input_features)
...
#     list_iter = list(data_g.nmi.unique())
#     data_g.set_index(['nmi','datetime'],inplace = True)
#     global data_gg
#     data_gg = copy.deepcopy(data_g)

#     predictions_prallel = pool_executor_parallel(run_forecast_pointbased_single_node_1,list_iter, input_features)
